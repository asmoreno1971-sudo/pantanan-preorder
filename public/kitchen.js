...
function notifyNewOrders(orders){
  ...
  if(seenOrderIds.size && newOrders.length){
    playAlertSound();
    newOrders
      .filter(order=>order.status === "Order Sent")
      .forEach((order,index)=>setTimeout(()=>printOrderSlip(order), index * 1000));
  }
...
function printOrderSlip(order){
  ...
  frame.title = "Pantanan order slip";
...
function orderSlipHtml(order){
  ...
  const items = order.items.map(item=>`
    <tr>
      ...
      <td>${item.qty}</td>
      <td>P${Number(item.subtotal) || 0}</td>
    </tr>
  `).join("");
  ...
  return `
    ...
    <html>
    <head>
      <title>Pantanan Order Slip</title>
      <style>
    ...
    <body>
      <div class="slip">
        <h1>Pantanan</h1>
        <div class="time">${escapeHtml(orderedAt)}</div>
        <div class="label">Order Summary for:</div>
        <div class="name">${escapeHtml(order.customerName)}</div>
        <div class="pickup-label">Order available at:</div>
        <div class="pickup">${escapeHtml(order.pickupTime)}</div>
        <table><tbody>${items}</tbody></table>
        <div class="total">Total P ${Number(order.total) || 0}</div>
      </div>
    </body>
    </html>
  `;
